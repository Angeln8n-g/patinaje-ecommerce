...
export function ProductReviews({ productId }: ProductReviewsProps) {
  ...
  useEffect(() => {
    fetchReviews();
    if (user) {
      hasPurchasedProduct(user.id, productId).then(setCanReview).catch(() => setCanReview(false));
    }
  }, [productId, user]);
  ...
  async function handleSubmitReview(e: React.FormEvent) {
    e.preventDefault();
    if (!user) { toast.error("Debes iniciar sesión para dejar una reseña"); return; }
    if (!newReview.comment.trim()) { toast.error("Por favor escribe un comentario"); return; }
    setSubmitting(true);
    try {
      const profile = await getProfile(user.id);
      const userName = profile ? `${profile.first_name || ""} ${profile.last_name || ""}`.trim() : user.email?.split("@")[0] || "Usuario";
      const review = await createProductReview({
        product_id: productId, user_id: user.id, user_name: userName || "Usuario",
  ...
  const averageRating = reviews.length > 0
    ? (reviews.reduce((acc, r) => acc + r.rating, 0) / reviews.length).toFixed(1) : "0.0";
    ...
    <div className="space-y-8 mt-12">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            <div className="flex items-center">
              {[1, 2, 3, 4, 5].map((star) => (
                <Star key={star} className={cn("h-4 w-4", star <= Number(averageRating) ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground")} />
              ))}
            </div>
      ...
      {!user ? (
        <div className="bg-muted/30 p-6 rounded-xl border text-center">
          ...
          <Button variant="outline" asChild><a href="/login">Iniciar Sesión</a></Button>
        </div>
      ) : !canReview ? (
            ...
            <div className="flex items-center">
              {[1, 2, 3, 4, 5].map((star) => (
                <button key={star} type="button" onClick={() => setNewReview({ ...newReview, rating: star })} className="hover:scale-110 transition-transform">
                  <Star className={cn("h-6 w-6 transition-colors", star <= newReview.rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/30")} />
                </button>
              ))}
            </div>
          </div>
          <Textarea placeholder="Comparte tu experiencia con este producto..." value={newReview.comment} onChange={(e) => setNewReview({ ...newReview, comment: e.target.value })} className="min-h-[100px] bg-background" />
          <Button type="submit" disabled={submitting} className="w-full md:w-auto font-black uppercase tracking-widest px-8">
          ...
          reviews.map((review) => (
            <div key={review.id} className="space-y-3 bg-card p-5 rounded-xl border shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star key={star} className={cn("h-3 w-3", star <= review.rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/20")} />
                  ))}
                </div>
              </div>
