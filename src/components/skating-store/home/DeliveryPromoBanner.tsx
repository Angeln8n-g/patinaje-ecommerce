...
export function DeliveryPromoBanner({ promo }: DeliveryPromoBannerProps) {
  ...
  return (
    <div className="container mx-auto px-4 mt-6 mb-8">
      <div 
        className="rounded-[32px] p-6 md:p-8 flex items-center justify-between relative overflow-hidden h-28 md:h-32"
      ...
      >
        ...
        <div className="flex items-center gap-3 md:gap-4 z-10 relative">
          <span className="font-bold text-xl md:text-2xl text-[#1A1A1A] drop-shadow-sm">{prefix}</span>
          <span className="bg-white border border-emerald-100 text-[#1A1A1A] px-3 py-1 md:px-4 md:py-1.5 rounded-full text-lg md:text-xl font-extrabold shadow-sm">
            {highlight}
          </span>
          <span className="text-[#1A1A1A] text-lg md:text-xl font-medium drop-shadow-sm">{suffix}</span>
        </div>
        ...
        {!imageUrl && (
          <div className="absolute right-0 top-0 h-full w-1/3 md:w-1/4 z-0">
             <div className="absolute right-[-20%] top-[-50%] w-[150%] h-[200%] bg-[#D4A574]/20 rounded-full blur-3xl"></div>
             <div className="absolute right-[10%] bottom-[-20%] w-24 h-24 bg-[#D4A574] rounded-full opacity-80 mix-blend-multiply"></div>
             <div className="absolute right-[-5%] top-[20%] w-16 h-16 bg-[#E8C4A0] rounded-full opacity-60"></div>
          </div>
        )}
      </div>
    </div>
